...
6. ~~Self-service registration~~ — **built (D-058)**, once its precondition (D-056) closed: `/profile/` now has a Create Account form alongside Sign In, with email verification before an account can sign in. Uses a dedicated bot-Administrator WordPress account to create other users — the one credential in this project with meaningfully more privilege than "act as one existing account" — since WordPress has no narrower built-in role for it and D-019 avoids adding one via custom code. Still open: resending a verification email if the first doesn't arrive.
