...
6. **Self-service registration — asked directly, deliberately deferred, precondition now met.** Right now every WordPress account is admin-provisioned (via WP-CLI) — there's no way for a visitor to create their own. Building it would mean a new endpoint holding an elevated, user-creating WP credential (a materially bigger security surface than anything built so far, which only ever verifies an *existing* account's own credentials). The stated reason to wait — no rate limiting yet — is now closed (D-056). Registration itself is still not built; this only removes the reason it was blocked, not a decision to build it next.
