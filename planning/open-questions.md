...
Duplicate is built ([decisions.md D-074](decisions.md#d-074-f-016-duplicate--scale-kept-as-its-own-follow-up-not-bundled-in)). Scale (a resize gesture distinct from move) remains open and undesigned: for a plain `rect`, "scale" plausibly means adjusting `size`; for a corner-reference-built polygon/polyline, it would mean moving every referenced corner outward from a pivot — a materially different mechanism. It also has no existing gesture to hang off of — would likely need new resize-handle UI, shown only once an element is selected, which is itself new interaction surface this app doesn't have yet.
...
## F-043 Keyboard-driven move/resize for the selected element, with a definable step size

Requested directly: once an element is selected, arrow keys should nudge its position, and Shift+arrow keys should resize it, with the step size per key press configurable somewhere (e.g. tied to a grid, F-031's own still-open grid-snap value).

Not designed at all yet: the default step size and where it's configured (plan-wide `settings`, tied directly to F-031 if that's ever built, or independent of it); which axis each arrow key affects (presumably up/down = Y, left/right = X); how resize-via-Shift+arrow works for a shape with no single `size` to resize (a corner-reference-built polygon/polyline — the same open question F-016's own "Scale" already raises); whether holding a key (key-repeat) commits one undo step per repeat or coalesces into one; and how this interacts with keyboard focus — arrow keys already mean something inside the code textarea (move the caret), so this needs a clear answer for when the shortcut should and shouldn't fire.
