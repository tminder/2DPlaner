...
Requested directly, not yet designed at all. A page/section listing external modules other authors have published, so a plan author can discover and pull one in via `module "<url>"` without already knowing a specific URL. Directly intersects F-009's still-open external-module trust question — a store that *lists* modules starts to look like an implicit endorsement of what's actually unsandboxed, unreviewed code, a materially different trust posture than "an AI happened to pick this URL." Not designed: where submissions/listings would live (this repo, a separate one, a form), whether anything is reviewed or verified before listing, how a module's own metadata/description is authored and by whom, and whether this changes F-009's own "gated by a confirm() dialog, no sandboxing" answer or just adds a discovery layer on top of the same trust model. Also now intersects [F-046](#f-046-how-ip-protection-and-eventual-commercial-success-actually-get-implemented) — a community module ecosystem needs its own ownership/licensing terms, not just a trust gate.

## F-046 How IP protection and eventual commercial success actually get implemented

Recorded directly alongside a new standing goal ([core-aims.md](core-aims.md), Core Aim 6): the project should eventually be commercially viable, with intellectual property protected accordingly. Nothing about *how* is designed yet. Concretely open, not yet even scoped:
- **Repo/source visibility.** The GitHub repo this deploys from has no `LICENSE` file today (confirmed by its absence) — meaning it's already "all rights reserved" by default, not accidentally permissive — but the repo's own visibility (public vs. private) hasn't been reconsidered against a commercial goal, and `docs/` being served straight from GitHub Pages means the app's full client-side source (the plan language's parser/renderer, every interaction mechanism) is trivially readable by any visitor regardless of the repo's own visibility.
- **What's actually the protectable asset.** The plan *language* and its parser/renderer, vs. the hosted *service* (cloud sync, accounts) built around it, vs. the "Planagonia" name/brand itself — these could plausibly want different protection strategies (e.g., an open language spec with a closed hosted service, matching how F-005's own scraping-protection reasoning already treats *content* vs. the language as separable, [decisions.md D-022](decisions.md#d-022-scraping-protection-content-not-the-language)).
- **A monetization shape isn't chosen** — nothing here implies free vs. paid, individual vs. team, self-hosted vs. cloud-only — commercial viability and IP protection are the standing goal; a specific business model is a separate, later decision this doesn't presuppose.
- **F-045's module store** would need explicit terms the moment it exists — who owns a submitted module, under what license it's listed, whether Planagonia itself can be forked using community modules as a shortcut around whatever protects the core.
- **Trademark/naming** for "Planagonia" itself hasn't been considered at all.

Not a decision to make preemptively — recorded so the next concrete choice that touches any of this (a new public-facing feature, F-045 itself, opening the repo further, a pricing page) gets weighed against it rather than decided in isolation.
