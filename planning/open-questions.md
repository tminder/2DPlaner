...
- **F-045's module store** would need its own explicit ownership/licensing terms the moment it exists — a listed module is someone else's code, not covered by this repo's own `LICENSE` at all; also still open whether Planagonia itself could be forked using community modules as a shortcut around whatever protects the core.
