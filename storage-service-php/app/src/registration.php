...
// server-side — never accepts a role from the caller. Returns WP's own {id, slug}.
function create_wp_user(array $config, string $username, string $email, string $password): array {
    ...
    if ($status !== 201) {
        // WP's own REST API returns a machine-readable error code (e.g.
        // "existing_user_login", "existing_user_email") — relayed as-is rather than
        // re-validated here, since WP is the actual source of truth for what's taken.
